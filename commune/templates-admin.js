(() => {
  ...
  function zoneName(t) { return t.zone_name || t.zone?.name || zones.find(z => String(z.id) === String(t.zone_id))?.name || "Zone inconnue"; }
  ...
  function renderToolbar() {
    ...
    [["", "Toutes les zones"], ...[...new Map(templates.map(t => [String(t.zone_id), zoneName(t)])).entries()].sort((a,b) => a[1].localeCompare(b[1], "fr"))].forEach(([v,l]) => { const o=document.createElement("option"); o.value=v; o.textContent=l; zone.appendChild(o); });
    ...
    [["name","Nom A → Z"],["zone","Zone A → Z"],["version","Version récente"],["updated","Dernière modification"]].forEach(([v,l]) => { const o=document.createElement("option"); o.value=v; o.textContent=l; sort.appendChild(o); });
  ...
  function render() {
    ...
    let list = templates.filter(t => {
      const hay = [t.name,t.slug,zoneName(t)].filter(Boolean).join(" ").toLowerCase();
    ...
    list.sort((a,b) => sortMode==="zone" ? zoneName(a).localeCompare(zoneName(b),"fr") || String(a.name||"").localeCompare(String(b.name||""),"fr") : sortMode==="version" ? Number(b.version||0)-Number(a.version||0) || String(a.name||"").localeCompare(String(b.name||""),"fr") : sortMode==="updated" ? String(b.updated_at||b.created_at||"").localeCompare(String(a.updated_at||a.created_at||"")) : String(a.name||a.slug||"").localeCompare(String(b.name||b.slug||""),"fr"));
    ...
    for (const t of list) {
      const row=document.createElement("div"); row.className="zone template-row";
      ...
      const meta=document.createElement("div"); meta.className="zone-meta"; meta.textContent=zoneName(t)+" · "+(t.slug||"—")+" · v"+(t.version??"—"); main.append(name,meta);
