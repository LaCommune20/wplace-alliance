(() => {
  ...
  function snapLngLat(lngLat) {
    ...
    for(const c of getCandidates()){
      if(c.type==="vertex"){
        ...
        const minX=Math.min(c.pa.x,c.pb.x)-SNAP_PX,maxX=Math.max(c.pa.x,c.pb.x)+SNAP_PX;
        const minY=Math.min(c.pa.y,c.pb.y)-SNAP_PX,maxY=Math.max(c.pa.y,c.pb.y)+SNAP_PX;
  ...
  function markSnapped(marker,index,hit){
    ...
    if(el){el.classList.add("snapped");el.title="Accroché à "+hit.kind+" : "+hit.label;}
  }

  function clearSnapped(marker,index){
    const el=marker?.getElement?.();
    if(el){el.classList.remove("snapped");el.title="Sommet "+(index+1);}
  }
  ...
  function bindMarker(marker,index){
    ...
    marker.on("drag",()=>{
      ...
      if(hit)markSnapped(marker,index,hit);else clearSnapped(marker,index);
    });
  }
  ...
  function installCheckbox(){
    ...
    if(stat)host.insertBefore(box,stat);else host.appendChild(box);
    box.querySelector("input").addEventListener("change",e=>{
      enabled=e.target.checked;
      invalidate();
      if(enabled){
        if(Array.isArray(markers))markers.forEach(bindMarker);
        if(typeof setState==="function")setState("Accrochage aux frontières activé.");
      }else if(typeof setState==="function")setState("Accrochage aux frontières désactivé.");
    });
  }
  ...
  function patchRenderMarkers(){
    ...
    // Important: the editor may already have rendered its initial markers before this module loads.
    if(Array.isArray(markers))markers.forEach(bindMarker);
  }
  ...
  function patchCreateClicks(){
    ...
    m.on("click",()=>{
      ...
      const index=pts.length-1,marker=markers[index];
      if(!marker)return;
      const hit=snapLngLat(marker.getLngLat());
      if(hit)markSnapped(marker,index,hit);
    });
  }
  ...
  function patch(){
    ...
    patchRenderMarkers();
    const m=getMap();
    if(m&&!m.__boundarySnapViewport){
      m.__boundarySnapViewport=true;
