(() => {
  ...
  function asGeoJSON(zones) {
    ...
    for (const z of zones) {
      ...
      features.push({
        type: "Feature",
        properties: {id: z.id, name: z.name || "Zone"},
  ...
  function renderReferenceZones() {
    ...
    m.addLayer({
      ...
      type: "fill",
      source: REF_SOURCE,
      paint: {"fill-color": "#111111", "fill-opacity": 0.035}
    }, before);
    m.addLayer({
      id: REF_LINE,
      type: "line",
      source: REF_SOURCE,
      paint: {"line-color": "#111111", "line-width": 2, "line-opacity": 0.58}
    }, before);
  }
  ...
  function installCheckbox() {
    ...
    box.querySelector("input").addEventListener("change",e=>{
      enabled=e.target.checked; invalidate();
      if(enabled){if(Array.isArray(markers))markers.forEach(bindMarker);renderReferenceZones();if(typeof setState==="function")setState("Accrochage aux frontières activé.");}
  ...
  function patchCreateClicks() {
    const m=getMap();if(!m||m.__boundarySnapCreate||mode!=="create")return;
    m.__boundarySnapCreate=true;
    m.on("click",()=>{if(!enabled)return;const pts=getPoints();if(!Array.isArray(pts)||!pts.length||typeof markers==="undefined"||!markers.length)return;const index=pts.length-1,marker=markers[index];if(!marker)return;const hit=snapLngLat(marker.getLngLat());if(hit)markSnapped(marker,index,hit);});
  }
  ...
  function patch() {
    ...
    patchCreateClicks();renderReferenceZones();loadZones();
  }
